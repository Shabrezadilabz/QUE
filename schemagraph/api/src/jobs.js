...
async function exportJobDbtLayer(
  ...
  if (format === 'dbt-pr') {
    ...
    const approvedJoinIds = Array.isArray(attestation.approvedRelationshipIds)
      ? attestation.approvedRelationshipIds
      : []
    const allowedBranches = String(settings.githubAllowedBranches || 'main')
      .split(/[\n,]+/)
      .map((s) => s.trim())
      .filter(Boolean)
    if (allowedBranches.length && !allowedBranches.includes(baseBranch)) {
      const err = new Error(
        `Base branch "${baseBranch}" is not in allowed deploy branches: ${allowedBranches.join(', ')}`,
      )
      err.status = 400
      throw err
    }
    const prBody = [
      ...
      `- **Approved join IDs:** ${approvedJoinIds.length ? approvedJoinIds.map((id) => `\`${id}\``).join(', ') : '_none_'}`,
      ``,
      ...
      `- [ ] Attestation fingerprint matches Que export audit`,
      ``,
